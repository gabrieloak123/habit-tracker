...
import { lte, and, gte, sql, eq, desc } from "drizzle-orm"
...
export async function getWeekSummary() {
    ...
    const goalsCompletedOnWeek = db.$with('goals_completed_on_week').as(
        db.select({
            ...
            title: goals.title,
            completedAtTime: goalCompletions.createdAt,
            completedAtDate: sql`
    ...
    const result = await db.with(goalsCreatedUpToWeek, goalsCompletedOnWeek, goalsCompletedByWeekDay)
        .select({
            completed: sql`(SELECT COUNT(*) FROM ${goalsCompletedOnWeek})`.mapWith(Number),
